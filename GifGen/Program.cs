...
namespace GifGen
{
    ...
    {
        /// <summary> 複数の画像をGIFアニメーションとして保存する </summary>
        ...
        /// <summary> Netscape Application Extensionブロックを返す。 </summary>
        /// <param name="loopCount"> 繰り返す回数。0で無限。 </param>
        ...
        /// <summary> Graphic Control Extensionブロックを返す。 </summary>
        /// <param name="delayTime"> 遅延時間（100分の1秒単位）。 </param>
        ...
        /// <summary> MemoryStreamの現在の位置から指定されたサイズのバイト配列を読み取る </summary>
    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            var targetDirs = new DirectoryInfo(args[0]).GetDirectories("*", SearchOption.TopDirectoryOnly).ToArray();
            if (!targetDirs.Any())
            {
                Console.WriteLine("target dirs not found.");
                Environment.Exit(1);
            }
        ...
        private static void Run(DirectoryInfo gifDir)
        {
            var images = gifDir.GetFiles("*", SearchOption.TopDirectoryOnly).Where(f => f.Extension.Equals(".gif", StringComparison.OrdinalIgnoreCase))
               .OrderBy(f => f.Name).Select(f => (Bitmap)Image.FromFile(f.FullName)).ToArray();

            // 設定はアズレンスタンプ合わせ
            UInt16 delay = 6;
            UInt16 loop = 0;
            var savePath = Path.Combine(gifDir.Parent.FullName, $"{gifDir.Name}.merged.gif");

            Generator.SaveAnimatedGif(savePath, images, delay, loop);
        }
    }
}
